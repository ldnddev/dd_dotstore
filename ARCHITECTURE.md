...
```
crates/
└── ldnddev_theme/     # in-repo YAML theme load/save + live editor
src/
...
- **Theme** — in-repo crate `crates/ldnddev_theme` loads/saves YAML and owns the live editor. Lookup `./dd_dotstore_theme.yml` → `$XDG_CONFIG_HOME/ldnddev/dd_dotstore_theme.yml` → built-in. Schema `version: 1`, every canonical color key, `#RRGGBB`. `F2` opens the editor (global save default).
...
- `i` / `E`        → import / export
- `F1`             → help
- `F2`             → theme editor (live preview; Tab local/global, default global; Y save; Esc/F2 revert)
- `F3`             → credits
